...
const BusinessContactsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        const response = await axios.get('http://localhost:3005/api/contactsBusiness', { withCredentials: true });
        setContacts(response.data);
      } catch (err) {
        setError('Error fetching business contacts');
      }
    };
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark p-6.5">
      <div className="border-b border-stroke py-4 px-6.5 dark:border-strokedark mb-6">
        <h3 className="font-medium text-black dark:text-white">Business Contacts</h3>
      </div>
