...
const CreateExpertisePage = () => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const response = await axios.post(`http://localhost:3005/api/CreateExpertiseData`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      navigate('/expertises');
      console.log('Expertise created successfully:', response.data);
    } catch (error: any) {
      console.error('Error creating expertise:', error.response ? error.response.data : error.message);
      setError(error.response ? error.response.data.error : 'Failed to create expertise');
    }
  };
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark p-6.5">
      <div className="border-b border-stroke py-4 px-6.5 dark:border-strokedark mb-6">
        <h3 className="font-medium text-black dark:text-white">Create Expertise</h3>
      </div>
