...
const CreateBannerDataPage: React.FC = () => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const response = await axios.post('http://localhost:3005/api/CreateBanner', {
        title,
        titleLogo,
        buttonLabel,
      });
      navigate('/banners');
      console.log('Banner data created successfully:', response.data);
    } catch (err: any) {
      console.error('Error creating banner data:', err.response ? err.response.data : err.message);
      setError(err.response ? err.response.data.error : 'Failed to create banner data');
    }
  };
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark p-6.5">
      <div className="border-b border-stroke py-4 px-6.5 dark:border-strokedark mb-6">
        <h3 className="font-medium text-black dark:text-white">Create Banner Data</h3>
      </div>
...
export default CreateBannerDataPage;
