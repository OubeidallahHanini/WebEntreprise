...
const UpdateHeroData: React.FC = () => {
  ...
  useEffect(() => {
    const fetchHeroData = async () => {
      try {
        ...
        setSectionHeadingTitle(data.section.headingTitle);
        setSectionParagraph(data.section.paragraph);
        setHeroContent(data.section.heroContent);
      } catch (error: any) {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      await axios.put(`http://localhost:3005/api/UpdateHeroData/${id}`, {
        title,
        caption,
        heroTitle,
        subHeadings,
      ...
      setError(error.response ? error.response.data.error : 'Failed to update hero data');
    }
  };
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark p-6.5">
      <h2 className="mb-6 text-2xl font-semibold">Update Hero Data</h2>
